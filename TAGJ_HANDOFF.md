...
2. **Never push to GitHub directly.** At session end, give Shaz a ready-to-paste Terminal command (`git` commands) to commit and push — he finds this easier than the old download/drag-to-GitHub-website method. Always provide the exact commands, not instructions to do it manually.
...
6. **Option B:** Feed recordings to ElevenLabs PVC → regenerate all chapters in Shaz's voice at scale
...
### 🔄 ElevenLabs PVC — شهباز صدا (IN TRAINING — 2026-09-12)

**Voice name:** شهباز صدا  
**Voice ID:** `0bEFxuaimRGqZGCP5LSf`  
**Language:** `fa` (Persian — set via API directly; not available in ElevenLabs UI)  
**Status:** ✅ Submitted for training 2026-09-12. Expected ready: 24–72 hrs.  
**Samples uploaded:** 9 files, ~1 hr 10 min total clean audio

| File | Duration | fal.ai CDN URL |
|------|----------|----------------|
| shaz_voice_00.mp3 | ~7:47 | https://v3b.fal.media/files/b/0aaa0d2d/JgYWWcclc20DEeKPcLHSW_1789166539369.mpeg |
| shaz_voice_01.mp3 | ~7:47 | https://v3b.fal.media/files/b/0aaa0d2f/qjwHEF5DjKxr0XHYSMqWa_1789166553764.mpeg |
| shaz_voice_02.mp3 | ~7:47 | https://v3b.fal.media/files/b/0aaa0d30/ba5iJAZm-kAQq3nEYXHy-_1789166562848.mpeg |
| shaz_voice_03.mp3 | ~7:47 | https://v3b.fal.media/files/b/0aaa0d30/Fl3Eic3INtpSNWv162j0O_1789166567346.mpeg |
| shaz_voice_04.mp3 | ~7:47 | https://v3b.fal.media/files/b/0aaa0d31/1EArNHl3FmzUwbhwo2xtU_1789166570226.mpeg |
| shaz_voice_05.mp3 | ~7:47 | https://v3b.fal.media/files/b/0aaa0d31/T8ZK9u8O4F5z-scEqc9JY_1789166574682.mpeg |
| shaz_voice_06.mp3 | ~7:47 | https://v3b.fal.media/files/b/0aaa0d31/h3PoFuKQ5skgWxSp0eBVq_1789166577400.mpeg |
| shaz_voice_07.mp3 | ~7:47 | https://v3b.fal.media/files/b/0aaa0d32/blkVZezT4eLUCU4G1M1qZ_1789166585993.mpeg |
| shaz_voice_08.mp3 | ~7:47 | https://v3b.fal.media/files/b/0aaa0d32/3kNlRi38AMiPLxlLP34Z0_1789166588756.mpeg |

**Note on language:** ElevenLabs PVC UI does not list Persian/Farsi. The correct `fa` code was set at voice creation via API (`POST /v1/voices/pvc`). All 9 samples passed upload with no language_mismatch — confirming `fa` is active internally. Do NOT change the language via the ElevenLabs UI (it would reset to Arabic and break uploads).

**Next after training completes:**
1. Generate prologue + ch1 as quality test
2. Check pronunciation, clarity, no background noise bleed
3. If approved → renew ElevenLabs plan (due before Sep 20) → generate all chapters
4. If not approved → assess whether more recordings needed

**Technical path used (for re-upload if ever needed):**
- Upload MP3s from Mac to fal.ai CDN via `mcp__remote-devices__fal-ai__upload_file` using Mac paths (`/Users/shahbazmirshahi/Desktop/TAGJ_Voice_Samples/`)
- Upload from CDN to ElevenLabs via Chrome browser JS (cloud container and Mac VM are both proxy-blocked for api.elevenlabs.io — Chrome is the only working path)
- Upload endpoint: `POST /v1/voices/pvc/{voice_id}/samples` with FormData field `files`
- Training endpoint: `POST /v1/voices/pvc/{voice_id}/train`
...
1. **ElevenLabs PVC training** — Submitted 2026-09-12. Wait 24–72 hrs for شهباز صدا (ID: `0bEFxuaimRGqZGCP5LSf`) to be ready.
2. **Test voice quality** — Once training complete, generate prologue + ch1. Check pronunciation, tone, no noise. Approve before renewing plan.
3. **Renew ElevenLabs** — Due before Sep 20. Only renew after voice test approved.
4. **Generate all chapters in شهباز صدا** — After plan renewal, generate all FA + EN chapters using `eleven_multilingual_v2`.
5. **Ch4 Logic Pro cleanup — RESUME NEXT SESSION** — Shaz recorded Chapter 4 in a library with Logic Pro + Apogee Duet 2. FX chain (from TAGJ_FX_Chain_Sheet.xlsx) is partially built. Status at session end:
   - ✅ Noise Gate: saved as preset `TAGJ_NoiseGate_Library` (Threshold -28dB, Hold 20ms, Release 207.6ms, Reduction -100dB, Hysteresis -3.0dB)
   - ✅ Channel EQ cuts: saved as preset `TAGJ_ChannelEQ_Cuts` (High-pass 80Hz, boxiness cut -3dB @ 250Hz)
   - ❌ PROBLEM: Noise Gate keeps disappearing from the insert chain — only Channel EQ visible in the slot. Root cause: Noise Gate must be in slot 1 (ABOVE the EQ), signal flows top to bottom. If NG is added after EQ it processes in wrong order and sounds wrong.
   - ❌ Still to add: Vintage VCA Compressor (step 3), DeEsser 2 (step 4), Channel EQ boosts (step 5), Phat FX (step 6, skip for audiobook export), Limiter -3dB (step 7)
   - ⚠️ RULE going forward: save each plugin preset immediately after dialling in. Do NOT wait until end.
   - Full FX chain spec is in `TAGJ_FX_Chain_Sheet.xlsx` (uploaded by Shaz — re-upload if needed)
   - Save two channel strip settings at end: `TAGJ_Narration_DRY` (no Phat FX) and `TAGJ_Narration_LUSH` (with Phat FX)
6. **Upload latest EPUBs to GitHub** — Both EN and FA corrected versions need uploading after 2026-08-20 session.
7. **Promotion execution** — start with WhatsApp/Telegram broadcast using the GitHub raw links.
8. **TAGJ_BOOK_RULES.md on GitHub** — still says .docx is working master. Clarify with Shaz.

---

## 📤 End-of-Session GitHub Push (Terminal Commands for Shaz)

At end of every session, give Shaz this block to copy-paste into Terminal (adjust filenames as needed):

```bash
cd ~/Desktop && git clone https://github.com/Shaz010/eee-journal.git 2>/dev/null || true
cd eee-journal
cp ~/Desktop/TAGJ_HANDOFF.md .
git add TAGJ_HANDOFF.md
git commit -m "Update TAGJ_HANDOFF.md - [brief description of session]"
git push
```

If other files were updated (EPUBs, etc.), add them before the commit:
```bash
cp ~/path/to/file.epub .
git add file.epub
```

**Do NOT give Shaz the old instructions** ("drag to GitHub website" / "upload manually"). Always give the terminal command block.
